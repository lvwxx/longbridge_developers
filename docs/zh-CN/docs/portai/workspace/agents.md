...
获取指定 Workspace 下的 Agent 列表。返回的 `uid` 即 [发起对话](/zh-CN/docs/portai/chat/conversation) 接口路径中的 Agent 标识；只有 `is_published` 为 `true` 的 Agent 才能发起对话。
...
| ∟ uid          | string   | true     | Agent UID，用于 [发起对话](/zh-CN/docs/portai/chat/conversation) 的路径参数 |
