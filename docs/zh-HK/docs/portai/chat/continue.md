...
當 [發起對話](/zh-HK/docs/portai/chat/conversation) 返回 `status = interrupted` 時，Agent 正等待你補充信息。通過本接口回傳答案，暫停的運行會從中斷處繼續執行，直到成功、再次中斷或失敗。
...
響應結構與 [發起對話](/zh-HK/docs/portai/chat/conversation#schemaconversation_response) 完全一致。
...
| ------ | ----------- | ----------------------------------------------------- |
| 200    | 返回成功    | [conversation_response](/zh-HK/docs/portai/chat/conversation#schemaconversation_response) |
